...
class TokenType:
    ...
    LPAREN = "LPAREN"  # Додано для лівої дужки
    RPAREN = "RPAREN"  # Додано для правої дужки
...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "*":  # Додано для розпізнавання операції множення
                self.advance()
                return Token(TokenType.MUL, "*")

            if self.current_char == "/":  # Додано для розпізнавання операції ділення
                self.advance()
                return Token(TokenType.DIV, "/")

            if self.current_char == "(":  # Додано для розпізнавання лівої дужки
                self.advance()
                return Token(TokenType.LPAREN, "(")

            if self.current_char == ")":  # Додано для розпізнавання правої дужки
                self.advance()
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            return self.visit(node.left) - self.visit(node.right)
        elif node.op.type == TokenType.MUL:  # Додано для обробки операції множення
            return self.visit(node.left) * self.visit(node.right)
        elif node.op.type == TokenType.DIV:  # Додано для обробки операції ділення
            return self.visit(node.left) / self.visit(node.right)
...
# Результат виконання коду з консолі:
